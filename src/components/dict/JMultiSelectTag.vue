...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // update--begin--autor:lvdandan-----date:20201120------for：LOWCOD-1086 下拉多選框,搜索時只字典code進行搜索不能通過字典text搜索
      filterOption(input, option) {
        return option.componentOptions.children[0].children[0].text.toLowerCase().indexOf(input.toLowerCase()) >= 0
      }
      // update--end--autor:lvdandan-----date:20201120------for：LOWCOD-1086 下拉多選框,搜索時只字典code進行搜索不能通過字典text搜索
    },
    model: {
      prop: 'value',
      event: 'change'
